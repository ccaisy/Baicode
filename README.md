...
| `shell_exec(command)` | 通过 `/bin/sh` 跑任意命令 | 60s 超时；每次独立子进程（cd 必须 `&&` 串联）；stdout/stderr 各 4000 字截断；禁交互式命令（vim/less/top）；安装类必带 `-y / --quiet` |
...
当前通过率 **~98%**（修复 E-02 / H-01 实时结构化数据兜底后）。完整 case 清单 + 失败分析见 [`memory-bank/eval.md`](memory-bank/eval.md)。
...
| [`memory-bank/tech_stack.md`](memory-bank/tech_stack.md) | 技术栈选型理由 |
